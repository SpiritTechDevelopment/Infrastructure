...
def _client_values(
    peer: dict[str, Any], metadata: dict[str, Any]
) -> tuple[list[str], list[str], str]:
    hub_interfaces = {
        environment: ipaddress.ip_interface(address)
        for environment, address in metadata["hub_addresses"].items()
    }
...
def execute(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="spiritvpn-platform-bootstrap-") as temporary:
        ...
        client_addresses, allowed_networks, internal_host = _client_values(peer, metadata)
        ...
        client_config = (
            ...
            f"Endpoint = {public_host}:{metadata['listen_port']}\n"
        ...
        runtime_variables["platform_wireguard_public_endpoint"] = (
            f"{public_host}:{metadata['listen_port']}"
        )
        _write_private(
