...
| Оверлейный адрес хаба | Grafana |
| Локальный SSH-туннель | Prometheus, Alertmanager, Loki — они слушают только петлю |

Grafana требует входа: анонимный доступ выключен намеренно, потому что на
оверлей выходят и ноды флота. Пароль администратора лежит на управляющем
сервере и в репозиторий не попадает.

Политика Vault выдачей оператора не затрагивается — см. раздел
[«Vault»](#14-vault).
