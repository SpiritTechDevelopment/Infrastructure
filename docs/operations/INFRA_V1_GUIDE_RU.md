...
контракт целиком, включая ростер операторов, из отревьюенного коммита.
...
Не редактируйте `/etc/spiritvpn/platform/runtime-vars.yml` вручную. Файл
перезаписывается из проверенного SOPS-контракта на каждом прогоне и служит
записью о прошлом применении: перед выкаткой `platform-deploy` сравнивает с ним
и печатает, какие поля контракта доступа меняет запуск. Печатаются только имена
полей — в проекции лежит токен бота.

Эта процедура больше не единственный способ донести контракт до хаба: с
2026-08-22 обычный `platform-deploy` применяет его целиком, включая ростер
операторов. Она остаётся для первичной настройки и для случая, когда хаб нужно
привести в порядок, не дожидаясь прогона.
...
Применяет их обычная выкатка платформы после слияния в `main`: для `develop`
она запускается сама, для `prod` её нужно запустить руками — автоматический путь
`prod` не выкатывает, потому что гейта одобрения не существует.

Пока прогон не прошёл, выдача не действует, а отзыв не состоялся: старый ключ и
peer продолжают работать. Отзыв, слитый но не выкаченный, выглядит в Git
выполненным и таковым не является — за прогоном нужно проследить.

Цена этой схемы названа прямо: **слитый в `main` коммит меняет и ростер SSH.**
Ревью по CODEOWNERS — единственное, что стоит между изменением и root на
управляющем сервере; отдельного подтверждения человеком с доступом больше нет.
