...
### Откуда берётся идентичность

Ключи оператора не выдаются «сверху»: вы создаёте их сами командой
`make operator-identity`, а действующий оператор добавляет в объявления только
их публичные части. Полная процедура — раздел
[«Выдача доступа новому оператору»](#выдача-доступа-новому-оператору). Пока
изменение не слито и не применено, ничего из описанного ниже работать не будет.

### Подключение к управляющему оверлею

Управляющий сервер и ноды достижимы только через оверлей. Публичных адресов у
управляющих интерфейсов нет, и на ping по публичному адресу узлы не отвечают —
это не признак поломки.

После слияния и применения выдачи заберите `main` и прочитайте контракт: в нём
публичный ключ хаба, его внешний эндпоинт, сеть оверлея и выданный вам адрес.
...
Соберите конфигурацию локально. Приватный ключ уже лежит у вас после
`make operator-identity`; из контракта берутся остальные значения.
...
Endpoint            = <внешний эндпоинт хаба>
AllowedIPs          = <сети оверлея из контракта>
PersistentKeepalive = 25
```

Не копируйте сюда значения из чужих примеров и не сохраняйте расшифрованный
контракт на диск. Поднимите туннель штатным способом вашей системы
(`wg-quick up`), после чего проверьте связность по оверлейному адресу хаба:

```bash
ping -c1 <оверлейный адрес хаба>
```

Echo внутри оверлея разрешён намеренно — это единственный путь, которым он
работает. Наружу узлы не отвечают и сами не пингуют.

### Внутренние имена

Grafana и Vault UI доступны по именам, а не по адресам. Получите строку для
`/etc/hosts` из контракта и вставьте её один раз:

```bash
make operator-hosts >> /etc/hosts
```

Имена живут под `.internal`. Этот суффикс зарезервирован ICANN под частное
использование и никогда не будет делегирован, тогда как произвольный TLD
однажды может стать настоящим — и тогда запрос, ушедший мимо туннеля, ответит
кто-то чужой. Тот же суффикс уже стоит в сертификате Vault, поэтому имя
работает без перевыпуска.

Службы DNS на управляющем сервере нет намеренно: ради двух имён это был бы ещё
один компонент и ещё одна поверхность на машине, которая держит Vault.

### Что доступно после подключения

| Через что | Что именно |
|---|---|
| SSH на управляющий сервер | обычные операторские действия, локальные SSH-туннели |
| `https://vault.management.internal:8200` | Vault UI |
| `http://grafana.management.internal:3000` | Grafana |
| Локальный SSH-туннель | Prometheus, Alertmanager, Loki — они слушают только петлю |

Оба интерфейса открыты **только операторским адресам**: firewall управляющего
сервера больше не доверяет оверлею целиком, поэтому нода флота до них не
дотягивается. Единственный порт, открытый всему оверлею, — приём логов Loki.

Grafana требует входа: анонимный доступ выключен намеренно. Пароль
администратора лежит на управляющем сервере и в репозиторий не попадает. Внутри
туннеля она работает по HTTP — WireGuard шифрует участок оператор—сервер, и
другие peer'ы этот трафик не видят.

Vault отдаёт TLS, подписанный локальным CA управляющего сервера. Его сертификат
нужно один раз добавить в доверенные; обращаться следует по имени, а не по
адресу — в SAN сертификата оверлейного адреса нет.

Наличие доступа к UI не означает прав внутри Vault: политика выдачей оператора
не затрагивается — см. раздел [«Vault»](#14-vault).
...
### Выдача доступа новому оператору

Доступ выдаётся в два конца, и приватный материал никогда не пересекает границу
между ними. Обратное направление — когда действующий оператор генерирует ключи
за нового и передаёт их — сломало бы три вещи сразу: шифровать такой набор не
для кого, приватный ключ навсегда осел бы в истории Git, а неотказуемость
исчезла бы, потому что ключами владели бы двое.

**Новый оператор, на своей машине.** Создаёт личность и запрос на включение:

```bash
make operator-identity OPERATOR=<идентификатор>
```

Приватные ключи — age, SSH и WireGuard — остаются в `~/.config/spiritvpn` с
правами `0600`. Наружу уходит только файл запроса с публичными частями; его
можно передать любым каналом, потому что важна целостность, а не секретность.
Команда печатает **отпечаток включения** — одну строку, покрывающую все три
ключа сразу.

**Действующий оператор.** Сначала сверяет отпечаток с владельцем ключей голосом
или лично. Это единственный якорь доверия во всей процедуре: без него выдача
включает того, кто перехватил запрос, а не того, кого ждут.

```bash
make operator-grant REQUEST=<файл> ADDRESS=<адрес в управляющем оверлее>
```

Адрес обязателен намеренно. Адреса нод флота живут в отдельных peer-фрагментах,
а не в контракте платформы, поэтому автоматический выбор мог бы молча
столкнуться с адресом ноды и сломать маршрутизацию оверлея. Выбирайте адрес из
диапазона, отведённого операторам.

Команда правит `.sops.yaml` и контракт платформы, перешифровывает все
затронутые файлы и **останавливается**. Дальше обычный путь: `make check`,
чтение диффа, коммит, pull request, ревью по CODEOWNERS. Применяется изменение
защищённым обновлением из этого же раздела:

```bash
scripts/platform-bootstrap.sh --reuse-tunnel --apply
```

**Новый оператор после слияния.** Забирает `main`, теперь расшифровывает
желаемое состояние и контракт платформы своим ключом, и собирает конфигурацию
WireGuard локально: приватный ключ у него уже есть, публичный ключ хаба и
выданный адрес читаются из контракта.
...
`AGE_RECIPIENT` обязателен, и это не формальность. Удаление оператора из ростера
закрывает SSH и оверлей, но не отнимает возможность расшифровывать: пока его
получатель остаётся в `.sops.yaml`, он продолжает читать всё желаемое состояние.
Отказ этот молчит и длится сколько угодно, поэтому команда без получателя
отзыв не выполняет.

Отозвать последнего оператора нельзя: доступ к платформе был бы утрачен целиком.

### Почему в Git нет ни одного приватного ключа

Зашифрованный бандл с ключами в репозитории выглядел бы удобнее, но означал бы
следующее: шифротекст остаётся в истории навсегда, доступный для офлайн-подбора
без ограничения скорости и без срока давности, а отзыв превращается из
отревьюенного диффа в ротацию всего, что бандл мог открыть. Поэтому в
объявлениях лежат только публичные части — то же правило записано и в
`.sops.yaml`.

`make check` сверяет набор получателей каждого файла с тем, что объявлено в
`.sops.yaml`. Именно эта проверка ловит забытый `sops updatekeys` — в обе
стороны, включая опасную, когда отозванный оператор продолжает расшифровывать.

Политику Vault ни одна из этих команд не трогает: она в контракте платформы не
описана и выдаётся отдельно.
