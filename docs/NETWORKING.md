...
Транспорт задаёт flow. У `tcp` это `xtls-rprx-vision`; у `xhttp` поле `flow` не
объявляется вовсе, зато обязательны `public.xhttp.path` и `public.xhttp.mode`.
В манифест бэкенду flow при этом уходит пустой строкой — в desired state его
нет, потому что sops не шифрует пустое значение и оно осталось бы открытым
скаляром в зашифрованном `spec`. Inbound
